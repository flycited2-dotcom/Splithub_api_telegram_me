...
их только читает. Исключение — **опт-цена Бриза и Русклимата**: оба идут через фид
Бриза, в БД сайта у них розница, поэтому опт (base/закупку) сервис берёт напрямую из
Бриз API (`stock_report_bot/breez.py`, по `nc_code`), с мягким откатом на цену из БД
при сбое/без ключа/если NC не в фиде. Daichi — опт из БД. Нужен `BREEZ_AUTH_HEADER`
