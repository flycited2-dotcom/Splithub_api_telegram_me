...
Отдельно от `specs.py` (тот — блок «ключевые особенности» серии из БД сайта +
УТП Бриза для Бриз/Daichi/Русклимата). JAC в БД сайта нет — его ТТХ приходят
файлом, формат и логика свои, поэтому отдельный модуль.
...
logger = logging.getLogger('stock_report_bot.jac_specs')
