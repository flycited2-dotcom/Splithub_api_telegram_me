...
# Часть поставщиков кодирует БЫТОВЫЕ модели ПЛОЩАДЬЮ помещения (м²), а не размером, и это
# число попадает в btu_calc/артикул как есть (50 и 70 вообще не бывают kBTU → точно площади).
# Карта площадь(м²)→типоразмер (kBTU) от владельца.
# ВАЖНО: в ПОЛУПРОМЕ (категория 6) 60 = реальные 60 000 BTU — там карту НЕ применяем.
_AREA_TO_SIZE = {25: 7, 30: 9, 35: 12, 50: 18, 60: 24, 70: 24}
_SEMI_INDUSTRIAL_CAT = 6


def _apply_area_map(n, category_id):
    """Бытовые/мобильные: число-площадь → типоразмер. Полупром: число = реальные kBTU."""
...
def size_from_btu(btu, category_id=None):
    ...
    Подстраховка: полные BTU (9000) → /1000; в бытовых число-площадь (25/30/35/50/60/70) →
    размер (в полупроме НЕ трогаем). None — если нет/мусор."""
    try:
        v = float(btu)
...
def _size_from_code(code, category_id=None):
    """Размер из кода модели/артикула (BSO-07HN8 → 7; в бытовых число-площадь → размер).
    None — нет кода."""
    m = _CODE_RE.search(code or '')
    if not m:
        return None
    return _apply_area_map(int(m.group(1)), category_id)
...
def build_channel_caption(positions, pct, brand, series, source,
    ...
    """
    # Категория серии (одна на серию): 6=полупром (60=реальные kBTU, карту площадей не
    # применяем), 2/7=бытовые/мобильные (число-площадь → размер).
    cat = next((r.get('category_id') for r in positions), None)
    ...
    for r in positions:
        ...
        if price is None:
            continue
        items.append((size_from_btu(r.get('btu_calc'), cat), price, _model_code(r, brand)))
    ...
    if items and (collision or not known):
        ...
        art = [_size_from_code(code, cat) for _, _, code in items]
